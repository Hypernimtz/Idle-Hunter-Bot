...
MAX_LOG_ENTRIES       = 50
INV_DISPLAY_MAX       = 10   # max items shown in menu/profile main
...
def get_total_boosts(user_id: str) -> dict:
    ...
    return {
        "luck": personal.get("luck", 0) + t_luck + prestige_b + tool_luck,
        "sell": personal.get("sell", 0) + t_sell + prestige_b,
        "xp":   personal.get("xp",   0) + t_xp   + prestige_b + tool_xp,
...
def has_unread_mail(user_id: str) -> bool:
    """Returns True if user has any unread mail."""
    d = data[user_id]
    # Tribe invite
...
def mail_tab_style(user_id: str, tab: str) -> int:
    """Returns button style: 4=red(unread), 3=green(active), 1=blue(inactive)."""
    d = data[user_id]
    has_unread = False
    if tab == "tribe":
...
def hunting_duration_str(joined_date_str: str) -> str:
    """Returns 'X years X months X days' since joined_date."""
    try:
...
def verify_needed_embed(user_id: str) -> discord.Embed:
    ...
    return discord.Embed(
        ...
        description=f"Use `/verify {code}` to continue.\nWe're preventing autoclickers.",
...
def run_hunt(user_id: str) -> dict:
    ...
    if now < cd:
        # Return remaining seconds so display is always positive
        remaining = cd - now
    ...
    add_log_entry(user_id, {
        "ts": int(now), "biome": biome, "tool": tool_name,
...
def build_menu_components(user_id: str, display_name: str) -> list:
    ...
    dropdown = {"type": 1, "components": [{
        ...
        "options": [
            ...
            {"label": "Shop",     "emoji": {"name": "🏪"},  "value": "shop",     "description": "Buy boosts and tools"},
            ...
            {"label": "Tools",    "emoji": {"name": "🔧"},  "value": "tools",    "description": "Equip your tools"},
            {"label": f"Mail{mail_indicator}", "emoji": {"name": "📬"}, "value": "mail", "description": "Check your mailbox"},
            {"label": f"Tribe", "emoji": {"id": "1500237653591851080", "name": "Bot_Tribe"
                        }, "value": "tribe", "description": "View your tribe"},
            {"label": f"Profile", "emoji": {"id": "1500237646121930863", "name": "User_Profile"
                        }, "value": "profile", "description": "View your profile"},
        ]
    }]}
...
def build_statistics_components(user_id: str, display_name: str) -> list:
    ...
    # Per-animal counts (only caught at least once)
    record   = d.get("record", {})
    animal_lines = []
    for animal, entry in sorted(record.items(), key=lambda x: x[1]["count"], reverse=True):
        animal_lines.append(
            f"-# {animal_emoji(animal)} **{entry['count']}×** {animal}"
        )
...
# ── TOOLS (equip only — buying in /shop) ──────
...
def build_shop_components(user_id: str, tab: str = "boosts") -> list:
    ...
    tab_row = {"type": 1, "components": [
        {"type": 2, "style": boost_btn_style, "label": "🧪 Boosts Shop",
         "custom_id": f"shop:tab:boosts:{user_id}", "flow": {"actions": []}},
        {"type": 2, "style": tools_btn_style, "label": "Tools Shop", "emoji": {"id": "1500237654891958394", "name": "Bot_Upgrade"},
...
def build_mail_components(user_id: str, tab: str = "tribe") -> list:
    ...
    # Override style to green (active)
    ...
    if tab == "tribe":
        ...
        if not gifts:
            ...
            for i, g in enumerate(gifts):
                ...
                g["read"] = True  # Mark as read on view
...
def build_tribe_components(user_id: str, tribe_name: str, page: str = "main", sort_mode: str = "rank") -> list:
    ...
    if page == "main":
        ...
        content = (
            ...
            f"-# {TRIBE_EMOJIS['kick']} Kick · {TRIBE_EMOJIS['invite']} Invite · "
            f"{TRIBE_EMOJIS['leave']} Leave · {TRIBE_EMOJIS['set_desc']} Set Desc · "
            f"{TRIBE_EMOJIS['ban']} Ban List · {TRIBE_EMOJIS['demote']} Demote · "
            f"{TRIBE_EMOJIS['leader']} Transfer"
        )
        row1 = []
        if is_leader or is_officer:
            row1 += [
                {"type": 2, "style": 4, "label": "Kick",
                 "emoji": {"id": "1500224528985620663"}, "custom_id": f"tribe:action:kick:{user_id}", "flow": {"actions": []}},
                {"type": 2, "style": 1, "label": "Invite",
                 "emoji": {"id": "1500224530348638329"}, "custom_id": f"tribe:action:invite:{user_id}", "flow": {"actions": []}},
            ]
        row1.append({"type": 2, "style": 4, "label": "Leave",
                     "emoji": {"id": "1500224526699597864"},
                     "custom_id": f"tribe:action:leave:{user_id}", "flow": {"actions": []}})
        if is_leader or is_officer:
            row1 += [
                {"type": 2, "style": 1, "label": "Set Desc",
                 "emoji": {"id": "1500237649112338592"},
                 "custom_id": f"tribe:action:set_desc:{user_id}", "flow": {"actions": []}},
                {"type": 2, "style": 4, "label": "Ban List",
                 "emoji": {"id": "1500224527756558497"},
                 "custom_id": f"tribe:action:banlist:{user_id}", "flow": {"actions": []}},
            ]
        row2 = []
        if is_leader:
            row2 += [
                {"type": 2, "style": 1, "label": "Demote",
                 "emoji": {"id": "1500237650366304378"},
                 "custom_id": f"tribe:action:demote:{user_id}", "flow": {"actions": []}},
                {"type": 2, "style": 1, "label": "Promote",
                 "emoji": {"id": "1500240066801569994"},
                 "custom_id": f"tribe:action:promote:{user_id}", "flow": {"actions": []}},
                {"type": 2, "style": 1, "label": "Transfer",
                 "emoji": {"id": "1500237652740538388"},
                 "custom_id": f"tribe:action:transfer:{user_id}", "flow": {"actions": []}},
            ]
        row2.append({"type": 2, "style": 2, "label": "◀ Back",
                     "custom_id": f"tribe:nav:main:{user_id}", "flow": {"actions": []}})

        comps = [
            {"type": 10, "content": content},
            {"type": 14, "divider": True, "spacing": 1},
            {"type": 1, "components": row1[:5]},
        ]
        if row2:
            comps.append({"type": 1, "components": row2[:5]})
        return [{"type": 17, "accent_color": _accent(user_id), "spoiler": False, "components": comps}]
...
def build_log_v2_components(user_id: str, page: int = 0) -> list:
    ...
    content = (
        ...
        f"-# <t:{ts}:F> · {TOOLS.get(tool,{}).get('emoji','🔧')} **{tool}** · {BIOME_NAMES.get(biome, biome)}\n\n"
...
async def on_interaction(interaction: discord.Interaction):
    ...
    if parts[0] == "gift":
        ...
        if parts[1] == "confirm":
            # gift:confirm:sender_id:recipient_id:format:amount:message
            recipient_id = parts[3]
            fmt          = parts[4]
            parsed       = int(parts[5])
            message      = ":".join(parts[6:])  # message may contain colons
            ...
            # Add gift mail entry for recipient
            gift_entry = {
                "sender_id":   owner_id,
    ...
    if parts[0] == "tribe":
        ...
        if action == "action":
            sub = parts[2]
            if sub == "kick":
                await interaction.response.send_modal(TribeKickModal(owner_id, tribe_nm)); return
            if sub == "invite":
            ...
            if sub == "banlist":
                await update_v2(interaction, build_tribe_components(owner_id, tribe_nm, "banlist", sort)); return
            if sub == "demote":
                await interaction.response.send_modal(TribeDemoteModal(owner_id, tribe_nm)); return
            if sub == "transfer":
                await interaction.response.send_modal(TribeTransferModal(owner_id, tribe_nm)); return
            if sub == "promote":
                await interaction.response.send_modal(TribePromoteModal(owner_id, tribe_nm)); return
            if sub == "leave":
    ...
    # ── TRIBE INVITE ACCEPT/DECLINE (old DM method) ───
...
class TribeKickModal(discord.ui.Modal, title="Kick Member"):
    uid_input = discord.ui.TextInput(label="User ID to kick", placeholder="123456789012345678", required=True, max_length=20)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if target in data:
            data[target]["tribe"] = None; data[target]["tribe_inv"] = None
        save_data_users(); save_data_tribe()
...
class TribeDemoteModal(discord.ui.Modal, title="Demote Officer"):
    uid_input = discord.ui.TextInput(label="Officer User ID to demote", placeholder="123456789012345678", required=True, max_length=20)
    def __init__(self, user_id, tribe_name):
        super().__init__(); self.user_id = str(user_id); self.tribe_name = tribe_name
    async def on_submit(self, interaction: discord.Interaction):
        target = self.uid_input.value.strip()
        td     = tribe_data[self.tribe_name]
        if target in td["roles"]["officer"]:
            td["roles"]["officer"].remove(target)
            td["roles"]["members"].append(target)
        save_data_tribe()
        await update_v2(interaction, build_tribe_components(self.user_id, self.tribe_name, "actions"))


class TribeTransferModal(discord.ui.Modal, title="Transfer Leadership"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.user_id not in td["roles"]["officer"]: td["roles"]["officer"].append(self.user_id)
        save_data_tribe()
        await update_v2(interaction, build_tribe_components(self.user_id, self.tribe_name, "main"))

class TribePromoteModal(discord.ui.Modal, title="Promote a Member"):
    uid_input = discord.ui.TextInput(label="Member User ID to Promote", placeholder="123456789012345678", required=True, max_length=20)
    def __init__(self, user_id, tribe_name):
        super().__init__(); self.user_id = str(user_id); self.tribe_name = tribe_name
    async def on_submit(self, interaction: discord.Interaction):
        target = self.uid_input.value.strip()
        td     = tribe_data[self.tribe_name]
        if target in td["roles"]["members"]:
            td["roles"]["members"].remove(target)
            td["roles"]["officer"].append(target)
        save_data_tribe()
        await update_v2(interaction, build_tribe_components(self.user_id, self.tribe_name, "actions"))
...
async def _common_init(interaction: discord.Interaction) -> str | None:
    if await check_maintenance(interaction):
        return None
...
async def hunt_cmd(interaction: discord.Interaction):
    ...
    if result.get("verify"):
        await interaction.response.send_message(embed=verify_needed_embed(user_id), ephemeral=True); return
    if result.get("tool_locked"):
...
async def mail_cmd(interaction: discord.Interaction):
    ...
    await send_v2(interaction, build_mail_components(user_id, "tribe"))
...
async def tribe_cmd(interaction: discord.Interaction):
    ...
    if not tribe_nm and tribe_inv and tribe_inv in tribe_data:
        await send_ephemeral_embed(interaction,
            f"You have a pending tribe invite! Use `/mail` to accept or decline.", discord.Color.yellow())
        return
    ...
    await send_v2(interaction, build_tribe_components(user_id, tribe_nm, "main"))
...
async def invite_cmd(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(embed=discord.Embed(
        ...
        description=f"[Click here to invite the bot!]({url1})\nClick here to get an invite to the support server! ({url2})",
